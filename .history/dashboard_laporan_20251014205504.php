...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ambil_laporan'])) {
    ...
    $nama_mekanik_list = $_POST['nama_mekanik'] ?? [];

    $nama_mekanik = implode(", ", array_map('htmlspecialchars', $nama_mekanik_list));

    // simpan nama mekanik ke kolom pesan_mekanik (atau bisa ke nama_mekanik kalau punya kolom sendiri)
    $sql = "UPDATE laporan 
            SET status='proses', pesan_mekanik='$nama_mekanik'
            WHERE id='$id_laporan'";
    ...
    if (!empty($mekanik_ids)) {
        ...
        $res = mysqli_query($conn, "SELECT nama FROM mekanik WHERE id IN ($id_str)");
    ...
    // simpan nama mekanik ke kolom pesan_mekanik (atau buat kolom baru nama_mekanik kalau mau)
    $sql = "UPDATE laporan 
            SET status='proses', pesan_mekanik='$nama_mekanik'
            WHERE id='$id_laporan'";
