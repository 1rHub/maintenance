...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ambil_laporan'])) {
    ...
    // ✅ Simpan ke laporan
    $sql = "UPDATE laporan 
            SET status='proses', mekanik_id=?, nama_mekanik=?, tanggal_ditunda=?
            WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssi", $mekanik_id_str, $nama_mekanik, $tanggal_ditunda ,$id_laporan);
