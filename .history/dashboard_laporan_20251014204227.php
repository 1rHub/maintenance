...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ambil_laporan'])) {
    ...
    if (empty($nama_mekanik)) {
        $res = mysqli_query($conn, "SELECT nama_mekanik FROM mekanik WHERE id='$id_laporan'");
...
<body>
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<label><input type="checkbox" name="nama[]" value="' . htmlspecialchars($row['nama']) . '"> ' . htmlspecialchars($row['nama']) . '</label>';
        }
    } else {
