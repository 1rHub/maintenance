...
if (isset($_POST['selesai_laporan'])) {
    ...
    $sql = "UPDATE laporan 
            SET status='finish', pesan_mekanik=?, tanggal_selesai=? 
            WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssi", $pesan_mekanik, $tanggal_selesai, $id_laporan);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tunda_laporan'])) {
    $id_laporan = $_POST['id_laporan'];
    $pesan_mekanik = $_POST['pesan_mekanik'];
    $tanggal_ditunda = date('Y-m-d H:i:s'); // simpan waktu ditunda
...
<body>
  ...
  <tr>
    ...
    SELECT l.id, l.tanggal, l.pesan, l.nama, l.divisi, l.kategori, 
           l.nama_mekanik, m.nama_mesin
    FROM laporan l
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<tr data-kategori='{$row['kategori']}'>
            ...
            <td class='kategori {$kategoriClass}'>" . ucfirst($row['kategori']) . "</td>
        </tr>";
    }
} else {
