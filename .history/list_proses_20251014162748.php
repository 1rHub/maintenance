...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['batal_proses'])) {
    $id_laporan = $_POST['id_laporan'];
    $sql = "UPDATE laporan SET status='kerusakan', nama_mekanik=NULL, pesan_mekanik=NULL WHERE id=?";
    $stmt = $conn->prepare($sql);
...
<body>
    ...
    SELECT laporan.*, mesin.nama_mesin 
    FROM laporan 
    JOIN mesin ON laporan.mesin_id = mesin.id 
    WHERE laporan.status IN ('proses','kerusakan')
